#!/usr/bin/env python3
"""
为每张卡生成个性化卡背（显示人物名字 + 赛季名称）
在 Docker 容器内执行: docker exec kmua-bot uv run python scripts/generate_personalized_card_backs.py [season_id]
"""
import asyncio
import math
import sys
...
def generate_card_back(character_name: str, season_name: str) -> Image.Image:
    """生成带人物名字和赛季名的卡背"""
    ...
    # === 8. 人物名字（主标题） ===
    char_font = _load_font(24, bold=True)
    char_bbox = draw.textbbox((0, 0), character_name, font=char_font)
    char_w = char_bbox[2] - char_bbox[0]
    draw.text(
        ((CARD_W - char_w) / 2, CARD_H / 2 - 20),
        character_name, fill=(220, 200, 130, 255), font=char_font
    )

    # === 9. 赛季名称（副标题） ===
    season_font = _load_font(16, bold=False)
    season_text = f"—— {season_name} ——"
    season_bbox = draw.textbbox((0, 0), season_text, font=season_font)
    season_w = season_bbox[2] - season_bbox[0]
    draw.text(
        ((CARD_W - season_w) / 2, CARD_H / 2 + 15),
...
async def main():
    ...
    if not cards:
        print(f"❌ 赛季 {season_id} 没有卡片")
        sys.exit(1)

    season_dir = BASE_DIR / f"season_{season_id}"
    season_dir.mkdir(parents=True, exist_ok=True)

    print(f"正在为赛季 {season_id}「{season.name}」生成 {len(cards)} 张个性化卡背...")

    for card in cards:
        back_img = generate_card_back(card.name, season.name)
        back_rgb = Image.new("RGB", (CARD_W, CARD_H), (30, 30, 30))
        back_rgb.paste(back_img, mask=back_img)

        out_path = season_dir / f"card_{card.card_number:02d}_back.png"
        back_rgb.save(out_path, "PNG")
        print(f"✅ card_{card.card_number:02d}_back.png - {card.name}")
